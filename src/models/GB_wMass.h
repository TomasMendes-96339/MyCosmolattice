#ifndef GB_wMass_H //Usual macro guard to prevent multiple inclusion
#define GB_wMass_H
...
// fldS : The actual object which contains the scalar fields.
  ...
  public:
    ...
    double xi,lambda, cutoff, H0, M,cStar;
    ...
    Model<MODELNAME>(parser,runPar.getLatParams(), toolBox, runPar.dt, STRINGIFY(MODELLABEL)) //MODELLABEL is defined in the cmake.
    {
      ...
      H0 = parser.get<double>("H0");	 
      cutoff = parser.get<double>("cutoff");
      M = parser.get<double>("M");

      cStar= sqrt(48*xi)*H0*H0/cutoff;
        ...
        alpha = 0;
        fStar = sqrt(48*xi)*H0*H0/cutoff;
        omegaStar = fStar;//;sqrt(lambda) * fStar;
   ...
   /////////
    ...
    auto potentialTerms(Tag<0>) // Quartic
    //
    ...
    {
        return lambda*0.25 * pow<4>(fldS(0_c));
    ...
    auto potentialTerms(Tag<1>) // Bare mass term
    {
        return  0.5*pow<2>(M/fStar)*pow<2>(fldS(0_c)) ;
    }
   ...
   /////////
    ...
    {
      return   lambda*pow<3>(fldS(0_c)) - 2*24*xi*pow<2>(cStar/cutoff)*pow<4>(aDotI/aI)  * (fldS(0_c)) + pow<2>(M/fStar)*(fldS(0_c));
    }	
   ...
   /////////
    ...
    {
      return  lambda*3 * pow<2>(fldS(0_c)) - 2*24*xi*pow<2>(cStar/cutoff)*pow<4>(aDotI/aI) + pow<2>(M/fStar);
    }

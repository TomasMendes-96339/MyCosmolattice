#ifndef TANH2HDF5_H //Usual macro guard to prevent multiple inclusion
#define TANH2HDF5_H
  ...
  public:
    ...
    Model<MODELNAME>(parser,runPar.getLatParams(), toolBox, runPar.dt, STRINGIFY(MODELLABEL)) //MODELLABEL is defined in the cmake.
    {
      ...
      /////////
        ...
        piS0 = parser.get<double, Ns>("initial_momenta", {0});
   ...
   /////////
    ...
    {
        return   2 * (M/phii) * pow<2>(tanh(fldS(0_c)*phii/M))/sinh(2*fldS(0_c)*phii/M);
    }
   ...
   /////////
    ...
    {
        return  4  * (2 - cosh(2*fldS(0_c)*phii/M) ) * pow<2>(tanh(fldS(0_c)*phii/M)) / pow<2>(sinh(2*fldS(0_c)*phii/M)) ;
    }

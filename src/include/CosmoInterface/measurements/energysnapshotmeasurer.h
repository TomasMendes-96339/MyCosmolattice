...
//HERE      
            saveScalarFld =  IsInContainer::check("E_S_F", toSave);  // field strength of the scalar singlets
...
//HERE
            if(saveScalarFld) {
                nameScalarFld = mRoot + "field_strength_snapshot_scalar.h5"; // name of the file
                fIO.saver.create( nameScalarFld  );
                fIO.saver.close();
            }
        ...
        {
//HERE
            if(saveScalarFld) {   // field strength of the scalar singlets
                ForLoop(i, 0, Model::Ns -1,
                                fIO.saver.open( nameScalarFld );
                                fIO.saver.save(t, model.fldS(i), "E_S_F_" + std::to_string(i));
                                fIO.saver.close();
                        );
            }

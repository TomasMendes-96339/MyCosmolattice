...
namespace TempLat {
    ...
    public:
        ...
        static inline auto  fld(Model& model, Tag<I> i)
        {
            return model.fldS(i);
        }
        ...
        //----------------ADDED BY TOMÁS ----------------------------------------------------------------------------------
        template<class Model, int I>  // <ff'> //remember that f'=pi/a^3 from the definition of pi
        static inline auto pi2S2(Model& model, Tag<I> i)
        {
            return  (model.piS(i))*model.fldS(i);
        }

        template<class Model, int I>  // <Grad[f]^2>
        static inline auto lap2S(Model& model, Tag<I> i)
        {
            return LatLapl<Model::NDim>(pow<2>(model.fldS(i)));
            //return 2*LatLapl<Model::NDim>(model.fldS(i))*model.fldS(i)+2*Grad2<Model::NDim>(model.fldS(i));
        }

        template<class Model, int I>  // <\chi\nabla^2\chi>
        static inline auto fldLapS(Model& model, Tag<I> i)
        {
            return model.fldS(i)*LatLapl<Model::NDim>(model.fldS(i));
        }

        template<class Model, int I>  // <f^2>
        static inline auto fld2S(Model& model, Tag<I> i)
        {
            return pow<2>(model.fldS(i));
        }
        
        template<class Model, int I>  // <f^4>
        static inline auto fld4S(Model& model, Tag<I> i)
        {
            return pow<4>(model.fldS(i));
        }

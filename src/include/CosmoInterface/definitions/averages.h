...
namespace TempLat {
    ...
    public:
        ...
        template<class Model> 
        static inline auto pi2S2(Model& model)  // <pi^2 [f]> (sum over fields)
        {
            return average(Total(i,0,Model::Ns - 1,FieldFunctionals::pi2S2(model,i)));
        }


        template<class Model>
        static inline auto lap2S(Model& model)  // <Grad^2[f]> (sum over fields)
        {
            return average(Total(i,0,Model::Ns - 1, FieldFunctionals::lap2S(model,i)));
        }

        template<class Model>
        static inline auto fldLapS(Model& model)  // <f*Grad^2[f]> (sum over fields)
        {
            return average(Total(i,0,Model::Ns - 1, FieldFunctionals::fldLapS(model,i)));
        }

        template<class Model>
        static inline auto fld2S(Model& model)  // f^2(sum over fields)
        {
            return average(Total(i,0,Model::Ns - 1, FieldFunctionals::fld2S(model,i)));
        }

        template<class Model>
        static inline auto fld4S(Model& model)  // f^4(sum over fields)
        {
            return average(Total(i,0,Model::Ns - 1, FieldFunctionals::fld4S(model,i)));
        }
        ...
        static inline void setAllAverages(Model& model) {
            ...
            if (model.Ns > 0) {
                ...
                model.pi2S2AvI = Averages::pi2S2(model); //ADDED BY TOMÁS
                model.grad2AvI = Averages::grad2S(model);
                model.lap2AvI = Averages::lap2S(model); //All ADDED BY TOMÁS
                
                model.Ppi2AvI = Averages::pi2S2(model);  //
                //model.Pgrad2AvI = Averages::grad2S(model); //
                model.Pfld4AvI = Averages::fld4S(model); //
                model.Plap2AvI = Averages::lap2S(model); //
                model.PfldPiAvI = Averages::pi2S2(model); //
                model.PfldlapAvI = Averages::fldLapS(model); //
                model.Pfld2AvI = Averages::fld2S(model); //
            }
            if (Model::NU1 > 0) {

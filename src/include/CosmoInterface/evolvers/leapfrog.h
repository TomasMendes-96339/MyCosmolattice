        ...
        void sync(Model& model, T tMinust0) {
            if(! synced) {
                ...
                if (expansion) {
                    ...
                    //if (fixedBackground) model.aDDotI = aBackground.ddot(tMinust0);
                    //if (fixedBackground) model.aDDotI = -0.5 * (1+0.333) * pow<2>(model.aDotI)/model.aI; //ADDED BY TOMÁS. ASSUMING w=0.333. Should generalize!
                }
